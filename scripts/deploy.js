/**
 * Weather Oracle Deployment Script
 * 
...
/**
 * Compiles the WeatherOracle.sol contract using solcjs
...
async function compileContract() {
  ...
  try {
    ...
    execSync('npx solcjs --bin --abi --include-path node_modules/ --base-path . -o ./build --optimize --optimize-runs 200 contracts/WeatherOracle.sol', {
...
async function checkExistingDeployment(networkName) {
  ...
  const deploymentFile = path.join(deploymentDir, 'WeatherOracle.json');
...
async function deployToNetwork(networkName) {
  ...
  const abiFile = files.find(file => file.includes('WeatherOracle.abi'));
  const binFile = files.find(file => file.includes('WeatherOracle.bin'));
  ...
  // Deploy contract
  console.log('Deploying WeatherOracle contract...');
  ...
  fs.writeFileSync(
    path.join(deploymentDir, 'WeatherOracle.json'),
 ...
 */
async function main() {
  console.log('=== WeatherOracle Deployment and Configuration ===');
